<script lang="ts">
    ...
    import ManageEmployeesModal from '$lib/components/ManageEmployeesModal.svelte';  // New modal for managing employees
    ...
    let showManageEmployeesModal = false; // For showing Manage Employees Modal
    let userName = '';
    ...
    onMount(async () => {
        ...
        if (user) {
            ...
            console.log('Fetched User Data:', userData); // Debugging user role
    ...
    function openManageEmployees() {
        showMenu = false;
        showManageEmployeesModal = true;  // Open Manage Employees Modal
    }
    ...
    function closeManageEmployeesModal() {
        showManageEmployeesModal = false; // Close Manage Employees Modal
    }
...
<main class="min-h-screen flex flex-col items-center justify-center space-y-6 p-8 relative">
    ...
    {#if showManageEmployeesModal} <!-- New Modal for Managing Employees -->
        <ManageEmployeesModal on:close={closeManageEmployeesModal} />
    {/if}
